...
var setZoneGenerationByGmeter = function(zone,zoneIteratorCB){
    ...
    var zoneCriteria = [
        { $match: { parameter : 'gridEnergyTotal', zone: zone._id, ts : ts   }  }, //TYPE Inverter
        { $project: { ts:1,device:1, deviceSn:1,site:1,zone:1,gateway:1,yield:{ $subtract: [ '$end', '$start' ] } } },
        { $group: { _id: {site:'$site',gateway:'$gateway',zone:"$zone", ts : '$ts'}, yield: { $sum: '$yield' } } },
        {$sort : {'_id.ts':1}}
    ];
    ...
    db.aggregate('hours',zoneCriteria, function(err, data) {
...
//            console.log("DATA LENGTH : ", data.length)
            ...
            var dataIterator = function(record, dataIteratorCB){
                ...
                console.log("Zone ", zone._id, " Generation by GMETER is ", record.yield, '@ : ', record._id.ts);
...
//                console.log("UPDATE ENERGY GEN : ", updateObj);

                db.update('energyGeneration', updateObj, dataIteratorCB);
...
//    console.log(ts)
    ...
    var zoneCriteria = [
        { $match: { parameter : 'invEnergyTotal', zone: zone._id, ts : ts }  }, //TYPE Inverter
         {$sort : {'ts':1}},
    ...
    db.aggregate('hours',zoneCriteria, function(err, data) {
        if(!err && data && data.length > 0){
            ...
            var dataIterator = function(record, dataIteratorCB){
                ...
                setTimeSlices.apply(record._id,[]);
                console.log("Zone ", zone._id, " Generation by Inverter is ", record.yield, '@ : ', record._id.ts);
